...
tape( 'the accumulator function skips updates when provided a non-numeric value', function test( t ) {
	var acc = incrnanmpcorr( 3 );
	var r;
	...
	t.equal( acc( '5.0', 1.0 ), r, 'returns current value when provided a non-numeric x' );
	t.equal( acc( 5.0, true ), r, 'returns current value when provided a non-numeric y' );
	t.equal( acc( [ 1.0 ], [ 2.0 ] ), r, 'returns current value when provided array values' );
	t.equal( acc( {}, {} ), r, 'returns current value when provided object values' );
	t.end();
});
